const techData = {
  ...
  python: {
    ...
    history: ` Python is a general-purpose programming language that is
        class-based, object-oriented[15] (although not a pure OO language,
        as it contains primitive types[16]), and designed to have as few
        implementation dependencies as possible. It is intended to let
        application developers write once, run anywhere (WORA),[17]
...
function trigger(event) {
  ...
  const currentTechData = techData[currentTech.trim()];
  if (!currentTechData) return (blog.innerHTML = '');
...
<div class="content">
  <h2>Tech's brief history:</h2>
  <h5>${currentTechData.history}</h5>
</div>

<h4>Created by: ${currentTechData.writer}</h4>`;
}
